...
pub fn run(o: &Common, detach: bool) -> i32 {
    ...
    if !opted_in(&manifest_dir) {
        dbg(&format!("no opt-in found from {}", manifest_dir.display()));
        return 0;
    }
    ...
    let dir = std::env::var("CRATEBANK_MANIFEST_DIR")
        .map(PathBuf::from)
        .unwrap_or(manifest_dir);
    let root = workspace_root(&dir);
    let root_s = root.to_string_lossy().to_string();
    dbg(&format!(
        "workspace root {root_s}; {} session log(s)",
        sessions().len()
    ));
    ...
    if let Some(pid) = std::env::var("CRATEBANK_WAIT_PID")
        .ok()
    ...
    if !wait_quiet(&newest, 1500, 600_000) {
        dbg("session never went quiet");
        return 0;
    }
    let Some(s) = read_session(&newest) else {
        dbg("could not parse session");
        return 0;
    };
    let mut s = s;
    let run_id = s.run_id.clone();
    if already_sent(&run_id) {
        dbg(&format!("{run_id} already sent"));
        return 0;
    }
    ...
    match post(&o.endpoint, &body) {
        Ok(_) => {
            mark_sent(&run_id);
            dbg(&format!("sent {run_id} -> {}", o.endpoint));
        }
