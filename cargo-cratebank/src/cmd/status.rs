...
pub fn run(o: &Common) -> i32 {
    let nightly = Command::new("cargo")
        .arg("-Zhelp")
        .output()
        .map(|x| String::from_utf8_lossy(&x.stdout).contains("build-analysis"))
        .unwrap_or(false);
    let s = sessions();
    println!("cargo home    {}", cargo_home().display());
    println!(
        "log dir       {}  ({} session log(s))",
        log_dir().display(),
        s.len()
    );
    println!(
    ...
    println!(
        "auto-send     {}",
        if opted_in(&here) {
            "ON for this project (share = true)"
        } else {
            "off here — run: cargo cratebank enable"
        }
    );
    let sent = std::fs::read_to_string(state_file())
        .map(|s| s.lines().count())
        .unwrap_or(0);
    println!("sessions sent {sent}");
    if let Some(p) = s.first() {
        println!("latest        {}", p.file_name().unwrap().to_string_lossy());
    }
    println!(
