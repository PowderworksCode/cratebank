...
fn codegen_value(args: &[String], key: &str) -> Option<String> {
    ...
    while let Some(a) = it.next() {
        let candidate = if a == "-C" { it.next()? } else if let Some(v) = a.strip_prefix("-C") { v } else { continue };
        if let Some(v) = candidate.strip_prefix(&want) { return Some(v.to_string()); }
    }
    None
}
        ...
        for e in rd.flatten() {
            ...
            for line in txt.lines() {
                if let Ok(v) = serde_json::from_str::<Value>(line) {
                    ...
                    if t >= window.0 && t <= window.1 { recs.push(v); }
                }
            }
        }
    }
    recs.sort_by(|a, b| a["started"].as_f64().unwrap_or(0.0)
                  .partial_cmp(&b["started"].as_f64().unwrap_or(0.0)).unwrap());

    let mut matched = 0;
    let mut total = 0;
    for ev in events.iter_mut() {
        if ev["reason"] != "unit-registered" { continue; }
        ...
        if ev["mode"] == "run-custom-build" { continue; }
        total += 1;
        let Some(name) = ev["target"]["name"].as_str().map(|s| s.replace('-', "_")) else { continue };
        ...
        let same_name = |r: &Value| r["crate_name"].as_str()
            .map(|c| c.replace('-', "_")) == Some(name.clone());
        let kind_ok = |r: &Value| {
            let ct = r["crate_type"].as_str().unwrap_or("");
            if want_bin { ct == "bin" } else { ct != "bin" }
        };
        let pos = recs.iter().position(|r| same_name(r) && kind_ok(r))
            .or_else(|| recs.iter().position(same_name));
        if let Some(pos) = pos {
            let r = recs.remove(pos);
            let (u, s) = (r["cpu_user_s"].as_f64().unwrap_or(0.0), r["cpu_sys_s"].as_f64().unwrap_or(0.0));
...
pub fn prune() {
    let Ok(rd) = std::fs::read_dir(sidecar_dir()) else { return };
    let day = std::time::Duration::from_secs(86_400);
    for e in rd.flatten() {
        if let Ok(md) = e.metadata() {
            if md.modified().ok().and_then(|m| m.elapsed().ok()).map(|d| d > day).unwrap_or(false) {
